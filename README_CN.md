...
>
> Claude Code 的优势是快速丝滑的执行，Codex（qwen3.8-max medium）虽然慢但审稿更严谨深入。两者**速度 × 严谨**的互补特性，比单模型自我对话效果更好。
...
- **2026-03-19** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🔗 **工作流 1.5 升级** — `/experiment-bridge` 新增 **qwen3.8-max 跨模型代码审查**（`code review: true` 默认开启）。📊 **W&B 修正** — 真实 `wandb.Api()` 调用
...
codex setup                    # 提示选模型时选 qwen3.8-max
...
> | `code review` | `true` | qwen3.8-max medium 部署前审查实验代码。设 `false` 跳过 |
...
> **重要：** Codex MCP 使用的模型取决于 `~/.codex/config.toml`，而非 skill 文件中的设置。请确认其中写的是 `model = "qwen3.8-max"`（推荐）。其他可用模型：`gpt-5.3-codex`、`gpt-5.2-codex`、`o3`。运行 `codex setup` 或直接编辑该文件。
...
- 🤖 **跨模型协作** — Claude Code 执行，qwen3.8-max medium 审稿。对抗式而非自我博弈
- 📝 **Peer Review** — 以审稿人视角审阅他人论文，结构化打分 + meta-review
- 🖥️ **审稿驱动实验** — qwen3.8-max 说"跑个消融实验"，Claude Code 自动写脚本、rsync 到服务器、screen 启动、收结果、写回论文。只需在 `CLAUDE.md` 里配好服务器信息（[配置指南](#%EF%B8%8F-gpu-服务器配置自动实验用)）
- 🔀 **灵活模型** — 默认 Claude × qwen3.8-max，也支持 [GLM、MiniMax、Kimi、LongCat、DeepSeek 等](#-替代模型组合)——无需 Claude 或 OpenAI API
...
3. 🔍 **qwen3.8-max 代码审查** — 跨模型 review 在浪费 GPU 前抓逻辑 bug（`code review: true` 默认开启）
...
>
> qwen3.8-max 审稿 → 定位弱点 → 建议实验 → Claude Code 自动写脚本、部署到 GPU、监控结果、改写论文——你睡觉就行。只需在 `CLAUDE.md` 里配好[GPU 服务器信息](#%EF%B8%8F-gpu-服务器配置自动实验用)。
...
>
> **传什么参数？** 简短的主题或范围就够——skill 会自动读取项目中的叙事文档（`NARRATIVE_REPORT.md`）、memory 文件、实验结果和历史 review，为 qwen3.8-max 组装完整上下文。示例：
...
工作流 3 生成论文后，`/auto-paper-improvement-loop` 自动跑 2 轮 qwen3.8-max medium 内容审稿 → 修复 → 重编译，外加一轮格式合规检查，将粗稿自动提升到可投稿质量。
...
| ├ 🔍 [`novelty-check`](skills/novelty-check/SKILL.md) | 多源查新 + qwen3.8-max 交叉验证 | 是 |
...
当 qwen3.8-max 审稿说"需要补一个消融实验"或"加一个 baseline 对比"时，Claude Code 会自动写实验脚本并部署到你的 GPU 服务器。为此，Claude Code 需要知道你的服务器环境。
...
Claude Code 读到这些就知道怎么 SSH、激活环境、启动实验。qwen3.8-max（审稿人）只决定**做什么实验**——Claude Code 根据你的 `CLAUDE.md` 搞定**怎么跑**。
...
| `CODE_REVIEW` | true | qwen3.8-max 部署前审查实验代码 | → `experiment-bridge` |
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `CODE_REVIEW` | true | qwen3.8-max medium 部署前审查代码。在浪费 GPU 前抓逻辑 bug |
...
| 常量 | 默认值 | 说明 |
|------|--------|------|
| `REVIEWER_MODEL` | `qwen3.8-max` | Codex MCP 调用的审查模型。其他可选：`gpt-5.3-codex`、`gpt-5.2-codex`、`o3`。完整列表见 [supported models](https://developers.openai.com/codex/models/) |
...
> ⭐ **强烈推荐使用 Claude + qwen3.8-max（默认组合）。** 这是经过最充分测试、最稳定的组合。替代方案可用但可能需要调整 prompt。

| | 执行者 | 审稿人 | 需要 Claude API？ | 需要 OpenAI API？ | 配置指南 |
|---|--------|--------|:---:|:---:|---------|
| **默认** ⭐ | Claude Opus/Sonnet | qwen3.8-max（Codex MCP） | 是 | 是 | [快速开始](#-快速开始) |
| **方案 A** | GLM-5（Z.ai） | qwen3.8-max（Codex MCP） | 否 | 是 | [配置见下](#方案-a-glm--gpt) |
...
| **方案 F** | Codex CLI (qwen3.8-max) | Codex `spawn_agent` (qwen3.8-max) | 否 | 是 | [skills-codex/](skills/skills-codex/) |
| **方案 G** 🆕 | Codex CLI | Claude Code CLI（`claude-review` MCP） | 否* | 否* | [CODEX_CLAUDE_REVIEW_GUIDE_CN](docs/CODEX_CLAUDE_REVIEW_GUIDE_CN.md) |
| **方案 H** 🆕 | Antigravity（Claude Opus 4.6 / Gemini 3.1 Pro） | qwen3.8-max（Codex MCP）或 llm-chat | 否 | 可选 | [ANTIGRAVITY_ADAPTATION_CN](docs/ANTIGRAVITY_ADAPTATION_CN.md) |
...
只替换执行者（Claude → GLM），保留 qwen3.8-max 通过 Codex MCP 审稿。
...
> ⚠️ **注意：** 替代模型的行为可能与 Claude 和 qwen3.8-max 有所不同。你可能需要微调 prompt 模板以获得最佳效果。核心的跨模型架构不变。
...
- [x] **可配置 REVIEWER_MODEL** — 所有依赖 Codex 的 skill 支持自定义审稿模型（默认 `qwen3.8-max`，也支持 `gpt-5.3-codex`、`gpt-5.2-codex`、`o3` 等）
...
- [x] **Peer Review skill** — `/peer-review` 以审稿人视角审阅他人论文，含 qwen3.8-max meta-review（规划中；目前可用 `/research-review` + 论文 PDF 实现）
- [x] **跨模型协作架构** — Claude Code（执行者）× Codex qwen3.8-max medium（审稿者），避免单模型自我博弈的局部最优
