...
def normalize_description(text: str) -> str:
    ...
    text = text.replace("via qwen3.8-max medium review", "via Claude review through claude-review MCP")
    return text
...
def transform_body(text: str) -> str:
    ...
    text = text.replace("qwen3.8-max medium", "Claude review")
    text = text.replace("Send the full paper text to qwen3.8-max medium:", "Send the full paper text to Claude through `claude-review`:")
    text = text.replace("Send the complete outline to qwen3.8-max medium for feedback:", "Send the complete outline to Claude for feedback:")
    ...
    text = text.replace("- **Use `send_input`** for Round 2 to maintain conversation context", "- **Use `mcp__claude-review__review_reply_start` plus `mcp__claude-review__review_status`** for Round 2 to maintain conversation context")
    text = text.replace("qwen3.8-max responses", "Claude reviewer responses")
