...
`auto-review-loop-minimax` 只是**一个** skill 的 MiniMax 版。项目中有 **12 个 skill** 使用 Codex MCP (`mcp__codex__codex`) 调用 qwen3.8-max 做审查。如果你想全面切换到 MiniMax，需要让 Claude Code 把它们全部改写。
