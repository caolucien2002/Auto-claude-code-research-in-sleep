...
refine-logs/EXPERIMENT_PLAN.md   →   implement → qwen3.7-plus review → deploy → collect → initial results ready
...
- **CODE_REVIEW = true** — qwen3.7-plus medium reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip.
...
Before deploying, send the experiment code to qwen3.7-plus medium for review:
