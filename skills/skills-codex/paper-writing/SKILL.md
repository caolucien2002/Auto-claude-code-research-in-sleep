...
- **REVIEWER_MODEL = `qwen3.8-max`** — Model used via a secondary Codex agent for plan review, figure review, writing review, and improvement loop.
...
- qwen3.8-max reviews the plan for completeness
...
- qwen3.8-max reviews figure quality and captions
...
- qwen3.8-max reviews each section for quality
...
**Round 1:** qwen3.8-max medium reviews the full paper → identifies CRITICAL/MAJOR/MINOR issues → Codex implements fixes → recompile → save `main_round1.pdf`

**Round 2:** qwen3.8-max medium re-reviews with conversation context → identifies remaining issues → Codex implements fixes → recompile → save `main_round2.pdf`
