...
User input (PROBLEM + vague APPROACH)
  ...
  -> Phase 2 (Codex/qwen3.8-max): Review for fidelity, specificity, contribution quality, and frontier leverage
...
- **REVIEWER_MODEL = `qwen3.8-max`** — Reviewer model used via Codex MCP.
...
Send the full proposal to qwen3.8-max for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
Send the revised proposal back to qwen3.8-max in the **same thread**:
...
[Full verbatim response from qwen3.8-max]
...
- **ALWAYS use `config: {"model_reasoning_effort": "medium"}`** for all Codex review calls.
