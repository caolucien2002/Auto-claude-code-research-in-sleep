...
- **REVIEWER_MODEL = `qwen3.7-plus`** — Model used via Codex MCP for brainstorming and review. Default reviewer model; can be changed to any model supported by your Codex setup.
...
1. **Novelty check**: Use the `/novelty-check` workflow (multi-source search + qwen3.7-plus cross-verification) for each idea

2. **Critical review**: Use qwen3.7-plus via `mcp__codex__codex-reply` (same thread):
   ```
...
3. **Combine rankings**: Merge your assessment with qwen3.7-plus's ranking. Select top 2-3 ideas for pilot experiments.
